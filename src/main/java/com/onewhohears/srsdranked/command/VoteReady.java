...
public class VoteReady {
    public static BrigadierCommand create(final SpeedrunShowdownRanked plugin) {
        LiteralCommandNode<CommandSource> main = BrigadierCommand.literalArgumentBuilder("vote_ready")
                ...
                .executes(context -> {
                    ...
                    if (serverName.equals(CONFIG.getString("default_server"))) {
                        context.getSource().sendMessage(errorMsg("Cannot Vote Ready in the Lobby!"));
                        return 0;
                    }
                    ...
                    if (plugin.voteReady(player, gameId)) return Command.SINGLE_SUCCESS;
                    return 0;
                })
                .build();
